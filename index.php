...
include 'connect.php'; 
?> 
<div class="category-heading pink">
  <h2>Seniori</h2><div class="line"></div>
</div>
<section class="seniori"> 
<?php 
$query = "SELECT * FROM vijesti WHERE arhiva=0 AND kategorija='seniori' LIMIT 3"; 
$result = mysqli_query($dbc, $query); 
       $i=0; 
       while($row = mysqli_fetch_array($result)) { 
             echo '<article>'; 
                echo'<div class="article">'; 
                echo '<div class="seniori_img">'; 
                echo $row['slika']; 
                echo '</div>'; 
...
<div class="category-heading blue">
  <h2>Juniori</h2><div class="line"></div>
</div>
<section class="juniori"> 
<?php 
$query = "SELECT * FROM vijesti WHERE arhiva=0 AND kategorija='juniori' LIMIT 3"; 
$result = mysqli_query($dbc, $query); 
       $i=0; 
       while($row = mysqli_fetch_array($result)) { 
             echo '<article>'; 
                echo'<div class="article">'; 
                echo '<div class="juniori_img">'; 
                echo $row['slika']; 
                echo '</div>'; 
                echo '<div class="media_body">'; 
                echo '<h4 class="title">'; 
